...
public class BotWellnessDecisioner implements Decision<AdvancedMurderBot.GameContext, BotMove> {
    ...
    public BotMove makeDecision(AdvancedMurderBot.GameContext context) {
        ...
        //my addition
        //if enuf gold mines wait near pub
        if(context.getGameState().getMe().getMineCount() >= (context.getGameState().getMines().size()/2))
        ...
        // Is the bot well?
        if(context.getGameState().getMe().getLife() >= 80) {
